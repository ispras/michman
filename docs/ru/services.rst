...
`CVAT`_ это инструмент для интерактивной аннотации видео и изображений для задач компьютерного зрения.
...
* **username** -- имя суперпользователя CVAT
* **email** -- почтовый адрес суперпользователя CVAT

.. note:: Пароль суперпользователя необходимо указать на CVAT-узле при помощи следующей команды: "docker exec -it cvat bash -ic 'python3 ~/manage.py createsuperuser'".
...
`Kubernetes`_  это открытое программное обеспечение для оркестрации контейнеризированных приложений — автоматизации их развёртывания, масштабирования и координации в условиях кластера.
...
* **enable_helm** -- определяет, будет ли установлен пакетный менеджер `helm`_.
...
* **enable_keystone_auth** -- определяет, будет ли доступна Keystone Webhook аутентификация и авторизация.
  ...
  Для доступа к kubernetes-кластеру через `kubectl` нужно:
  * Установить `client-keystone-auth` from `GitHub`_
  * Указать пользовательскую конфигурацию в `KUBECONFIG` для того, чтобы этот клиент получил токен из Kubernetes
...
Подробнее в документации `k8s-keystone-auth`_.
